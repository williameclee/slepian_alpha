...
function varargout = plotcont(c11, cmn, res, lonofs, pcol, lolax)
    ...
    switch res
        case {0, 7, 9}
            fid = fopen(fullfile(dataFolder, 'cont.mtl'), 'r', 'b');
            cont = fread(fid, [5217 2], 'uint16');
            fclose(fid);
        case {1, 3, 4, 11}
            fid = fopen(fullfile(dataFolder, 'cost.mtl'), 'r', 'b');
            cont = fread(fid, [9598 2], 'uint16');
            fclose(fid);
        case 2
            load(fullfile(dataFolder, 'conm'))
            hold on
            ...
            XYZ = [conxm conym];
        case 10
            load(fullfile(dataFolder, 'conmg'))
            hold on
            handl{1} = plot(conxm, conym, 'k');
            axlim = [];
            handl{2} = plot(xbox, ybox, 'k');
            XYZ = [conxm conym];
        case 5
            cont = load(fullfile(dataFolder, 'California'));
            cont = cont.cont;
        case 8
            cont = load(fullfile(dataFolder, '1to5M'));
            cont = cont.cont;
    ...
    switch res
        case {0, 1, 5, 8}
            ...
            hold on
            handl = plot(lon + lonofs, lat, 'k', 'Linewidth', 1);
            isValidPoint = ~isnan(lon) & ~isnan(lat);
            axlim = ...
                [min(lon(isValidPoint)), max(lon(isValidPoint)), ...
                 min(lat(isValidPoint)), max(lat(isValidPoint))];
            XYZ = [lon + lonofs, lat];
        case 7
        ...
        case 9
            ...
            hold off
            set(pc, 'Color', 'k', 'lines', '-', 'marker', 'none')
            delete(cat(1, pgc{:}))
            handl = pc;
            % XYZ=[lon+ofs lat];
        ...
        case 11
            ...
            [xv, yv, zv] = sph2cart(deg2rad(lon), deg2rad(lat), 1);
