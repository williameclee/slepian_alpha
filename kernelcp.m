...
function varargout = kernelcp(Lmax, dom, pars, ngl, rotb)
    ...
    if isstr(dom)
        ...
        switch dom
            ...
            otherwise
                filname = sprintf('WREG-%s-%i.mat', dom, Lmax);
                ...
                if (strcmp(dom, 'antarctica') || strcmp(dom, 'contshelves')) && rotb == 1
                    filname = sprintf('WREG-%s-%i-%i.mat', dom, Lmax, rotb);
                end
    ...
    elseif iscell(dom)
        ...
        if dom{2} == 0
            ...
            h = [dom{1} num2str(dom{2})];
        end

        buf = dom{2};
        % However, if dom{2} turns out to be zero, we should ignore it.
        filname = sprintf('WREG-%s-%i.mat', h, Lmax);
    else
        ...
        h = hash(dom, 'sha1');
        filname = sprintf('%s-%i.mat', h, Lmax);
    end

    fnpl1 = sprintf('%s/%s', filoc1, filname);
    fnpl2 = sprintf('%s/%s', filoc2, filname);

    if exist(fnpl1, 'file') == 2 && ~isstr(ngl)
        % Check the KERNELC directory
        load(fnpl1)
        fprintf('%s loading %s\n', upper(mfilename), fnpl1)
    elseif exist(fnpl2, 'file') == 2 && ~isstr(ngl)
        % Check if you have a file in the old KERNELCP directory
        load(fnpl2)
        fprintf('%s loading %s. Consider moving your kernel files back to the KERNELC directory\n', upper(mfilename), fnpl2)
    else
        ...
        fprintf('KERNELCP (Matrix)  took %8.4f s\n', etime(clock, t0))
